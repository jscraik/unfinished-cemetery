...
## Purpose
Define a single canonical content contract for files in `/_headstones/`.
...
Defined in `/_data/headstone-taxonomy.yml`:
- `scope-explosion`
- `doubt`
- `confusion`
- `theory-trap`
...
## Validation Rules
- Frontmatter keys/types are validated against `/_data/headstone-schema.yml`.
- Cause values must exist in `/_data/headstone-taxonomy.yml`.
